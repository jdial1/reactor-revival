...
| 4   | **The "Morning Coffee" Reward & Deterministic Trust** | Offline progression is mathematically flawless (exact tick simulation or proven approximations). Returning after 10 hours yields a curated, verifiable upgrade spree. Welcome-back must project meltdown risk before Fast-Forward — §9 #8. |
...
| **Secret sauce**        | **The "Morning Coffee" reward.** Impeccably tuned offline progression made opening the game the first thing players did when they woke up. A 10-hour offline gap provided exactly enough resources for a massive, satisfying upgrade spree.                                                                                           |
| **Revival application** | `processOfflineTime` must be mathematically flawless. The welcome-back UI must clearly display exactly what was accomplished while away — maximizing the dopamine hit of returning to the tab. Offline help copy **shipped** (Tier S). Steal challenge-map modifiers and automation framing; **skip** monetization patterns entirely. |
