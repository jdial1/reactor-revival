...
export class ObjectiveManager {
  ...
  set_objective(objective_index, skip_wait = false) {
    console.log(`[DEBUG] set_objective called with index: ${objective_index}, skip_wait: ${skip_wait}`);

    // Prevent setting objectives before data is loaded
    if (!this.objectives_data || this.objectives_data.length === 0) {
    ...
    // Handle negative indices by clamping to 0
    if (objective_index < 0) {
      console.warn(`[DEBUG] Objective index ${objective_index} is negative. Clamping to 0.`);
      objective_index = 0;
    }

    // Safeguard: Ensure objective index is within valid bounds
    // Allow index to reach the "All objectives completed!" objective (length - 1) for natural progression
    // But clamp if it goes beyond that (which would be invalid)
    const maxValidIndex = this.objectives_data.length - 1; // Allow reaching "All objectives completed!"
