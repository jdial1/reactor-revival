export default [
    ...
    },
    {
        ...
        levels: 1,
        onclick: function(upgrade, game) {
            // game.heat_controlled is set in app.js via window.enable/disable_heat_control
        }
    },
    {
        ...
        levels: 1,
        onclick: function(upgrade, game) {
            game.heat_outlet_controlled = (upgrade.level > 0);
        }
    },
    {
        id: 'improved_piping',
    ...
    },
    {
        ...
        onclick: function(upgrade, game) {
            ...
            for (var i = 1; i <= 6; i++) {
                part = game.part_objects && game.part_objects['reactor_plating' + i];
                if (part) {
                    part.reactor_heat = part.part.base_reactor_heat * (upgrade.level + 1) * Math.pow(2, quantumBufferingLevel);
    ...
    },
    {
        ...
        onclick: function(upgrade, game) {
            ...
            const quantumBufferingLevel = (game.upgrade_objects && game.upgrade_objects['quantum_buffering'] && game.upgrade_objects['quantum_buffering'].level) || 0;
            for (var i = 1; i <= 6; i++) {
                part = game.part_objects && game.part_objects['capacitor' + i];
                if (part) {
    ...
    },
    {
        ...
        onclick: function(upgrade, game) {
            ...
            for (var i = 1; i <= 6; i++) {
                part = game.part_objects && game.part_objects['coolant_cell' + i];
                if (part) {
    ...
    // Reflectors
    {
        ...
        onclick: function(upgrade, game) {
            var part;
            for ( var i = 1; i <= 6; i++ ) {
                part = game.part_objects && game.part_objects['reflector' + i];
                if (part) {
    ...
    // Exchangers
    {
        ...
        onclick: function(upgrade, game) {
            ...
            for ( var i = 1; i <= 6; i++ ) {
                part = game.part_objects && game.part_objects['heat_inlet' + i];
                if (part) {
                    part.transfer = part.part.base_transfer * (upgrade.level + 1) * Math.pow(2, game.upgrade_objects && game.upgrade_objects['fluid_hyperdynamics'] && game.upgrade_objects['fluid_hyperdynamics'].level);
                    part.updateDescription();
                }

                part = game.part_objects && game.part_objects['heat_outlet' + i];
                if (part) {
                    part.transfer = part.part.base_transfer * (upgrade.level + 1) * Math.pow(2, game.upgrade_objects && game.upgrade_objects['fluid_hyperdynamics'] && game.upgrade_objects['fluid_hyperdynamics'].level);
                    part.updateDescription();
                }

                part = game.part_objects && game.part_objects['heat_exchanger' + i];
                if (part) {
                    part.transfer = part.part.base_transfer * ( upgrade.level + 1 ) * Math.pow(2, game.upgrade_objects && game.upgrade_objects['fluid_hyperdynamics'] && game.upgrade_objects['fluid_hyperdynamics'].level);
    ...
    // Vents
    {
        ...
        onclick: function(upgrade, game) {
            var part;
            for ( var i = 1; i <= 6; i++ ) {
                part = game.part_objects && game.part_objects['vent' + i];
                if (part) {
    ...
    },
    {
        ...
        onclick: function(upgrade, game) {
            ...
            for ( var i = 1; i <= 6; i++ ) {
                part = game.part_objects && game.part_objects['capacitor' + i];
                if (part) {
                    part.reactor_power = part.part.base_reactor_power * (game.upgrade_objects && game.upgrade_objects['improved_wiring'] && game.upgrade_objects['improved_wiring'].level + 1) * Math.pow(2, upgrade.level);
                    part.containment = part.part.base_containment * (game.upgrade_objects && game.upgrade_objects['improved_wiring'] && game.upgrade_objects['improved_wiring'].level + 1) * Math.pow(2, upgrade.level);
                    part.updateDescription();
                }

                part = game.part_objects && game.part_objects['reactor_plating' + i];
                if (part) {
                    part.reactor_heat = part.part.base_reactor_heat * (game.upgrade_objects && game.upgrade_objects['improved_alloys'] && game.upgrade_objects['improved_alloys'].level + 1) * Math.pow(2, upgrade.level);
                    part.updateDescription();
                }
            }
        }
    ...
    },
    {
        ...
        onclick: function(upgrade, game) {
            ...
            for ( var i = 1; i <= 6; i++ ) {
                part = game.part_objects && game.part_objects['heat_inlet' + i];
                if (part) {
                    part.transfer = part.part.base_transfer * (game.upgrade_objects && game.upgrade_objects['improved_heat_exchangers'] && game.upgrade_objects['improved_heat_exchangers'].level + 1) * Math.pow(2, upgrade.level);
                    part.updateDescription();
                }
                ...
                part = game.part_objects && game.part_objects['vent' + i];
                if (part) {
                    part.vent = part.part.base_vent * (game.upgrade_objects && game.upgrade_objects['improved_heat_vents'] && game.upgrade_objects['improved_heat_vents'].level + 1) * Math.pow(2, upgrade.level);
                    part.updateDescription();
                }
            }
        }
    },
    {
        ...
        onclick: function(upgrade, game) {
            ...
            for ( var i = 1; i <= 6; i++ ) {
                part = game.part_objects && game.part_objects['vent' + i];
                if (part) {
                    part.containment = part.part.base_containment * (game.upgrade_objects && game.upgrade_objects['improved_heat_vents'] && game.upgrade_objects['improved_heat_vents'].level + 1) * Math.pow(2, upgrade.level);
                    part.updateDescription();
                }

                part = game.part_objects && game.part_objects['heat_exchanger' + i];
                if (part) {
                    part.containment = part.part.base_containment * (game.upgrade_objects && game.upgrade_objects['improved_heat_exchangers'] && game.upgrade_objects['improved_heat_exchangers'].level + 1) * Math.pow(2, upgrade.level);
                    part.updateDescription();
                }
            }
        }
    ...
    },
    {
        ...
        onclick: function(upgrade, game) {
            var part;
            for ( var i = 1; i <= 3; i++ ) {
                part = game.part_objects && game.part_objects['protium' + i];
                if (part) {
                    part.base_heat = part.part.base_heat * Math.pow(2, upgrade.level) * Math.pow(2, game.upgrade_objects && game.upgrade_objects['unleashed_cells'] && game.upgrade_objects['unleashed_cells'].level);
                    part.heat = part.part.heat * Math.pow(2, upgrade.level) * Math.pow(2, game.upgrade_objects && game.upgrade_objects['unleashed_cells'] && game.upgrade_objects['unleashed_cells'].level);
                    part.base_power = part.part.base_power * (game.upgrade_objects && game.upgrade_objects['infused_cells'] && game.upgrade_objects['infused_cells'].level + 1) * Math.pow(2, upgrade.level) * Math.pow(2, game.upgrade_objects && game.upgrade_objects['unleashed_cells'] && game.upgrade_objects['unleashed_cells'].level);
                    part.power = part.part.power * (game.upgrade_objects && game.upgrade_objects['infused_cells'] && game.upgrade_objects['infused_cells'].level + 1) * Math.pow(2, upgrade.level) * Math.pow(2, game.upgrade_objects && game.upgrade_objects['unleashed_cells'] && game.upgrade_objects['unleashed_cells'].level);
