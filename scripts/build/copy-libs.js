  ...
  setup(buildApi) {
    ...
    buildApi.onLoad({ filter: /.*/, namespace: "node-stub" }, (args) => {
      if (args.path === "node:fs/promises") {
        return {
          contents: "export async function readFile(){throw new Error('node:fs unavailable in browser');}",
          loader: "js",
        };
      }
      if (args.path === "node:url") {
        return {
          contents: "export function fileURLToPath(){throw new Error('node:url unavailable in browser');}",
          loader: "js",
        };
      }
      if (args.path === "node:path") {
        return {
          contents: "export function dirname(){return '.';}export function join(...p){return p.filter(Boolean).join('/');}",
          loader: "js",
        };
      }
