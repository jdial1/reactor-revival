...
import { syncReactorHeatVisualDom, isHeatNetBalanced } from "../heatDomSync.js";
import {
...
export class HeatVisualsUI {
  constructor(ui) {
    ...
    if (st) {
      ...
      subscribeKey(st, "stats_net_heat", () => {
  ...
  drawHeatFlowOverlay() {
    ...
    if (!engine || typeof engine.getLastHeatFlowVectors !== "function") {
      render(html``, host);
      return;
    }
...
export class GridInteractionUI {
  ...
  _wireTileFx() {
    ...
    if (!uiState) return;
    subscribeKey(uiState, "tile_fx", (items) => {
  ...
  _wireVisualFx() {
    const uiState = this.ui?.uiState;
    if (!uiState) return;
    subscribeKey(uiState, "visual_fx", (fx) => {
      if (!fx?.length) return;
