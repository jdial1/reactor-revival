...
import { readObjectiveProgress } from "../domain/objectives.js";
...
export function formatObjectiveDisplayInfo(manager) {
...
export function getObjectiveClaimText(reward) {
  const rewardLabel = formatObjectiveRewardLabel(reward);
  return rewardLabel ? `Claim ${rewardLabel}` : "Claim";
}

export function formatCurrentObjectiveInfo(manager) {
  const def = manager?.current_objective_def;
  return {
    index: manager?.current_objective_index ?? 0,
    title: def
      ? (typeof def.title === "function" ? def.title() : def.title)
      : "No objective loaded",
    checkId: def?.checkId || null,
    total_objectives: manager?.objectives_data?.length ?? 0,
